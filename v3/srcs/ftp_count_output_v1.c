...
/*   Updated: 2022/06/15 10:46:43 by hnoguchi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ssize_t	ftp_strclen(const char *s, char c)
{
	size_t	len;
...
char	*ftp_strndup(const char *s1, ssize_t len)
{
	char	*temp;
...
int	ftp_count_output(const char *save, va_list args)
{
	...
	while (save[i] != '\0')
	{
		...
		if (save[i] == '%')
		{
			...
			buf = ftp_treat_something(save[i], args, &total_len, &buf_len);
			if (buf_len == -1)
				return (-1);
			// total_len += buf_len;
		}
